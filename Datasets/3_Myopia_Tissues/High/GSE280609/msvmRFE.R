...
# Makes a plot of average generalization error vs. number of top features
    ...
    AddLine(errors)
    if(!is.null(errors2)) AddLine(errors2, '#2BA2DE')
    abline(h=no.info, lty=3)
}

Plotaccuracy <- function(accuracy, accuracy2=NULL, no.info=0.5, ylim=range(c(accuracy, accuracy2), na.rm=T), xlab='Number of Features',  ylab='10x CV Accuracy') {
# Makes a plot of average generalization error vs. number of top features
    AddLine <- function(x, col='black') {
        lines(which(!is.na(accuracy)), na.omit(x), col=col)
        points(which.min(x), min(x, na.rm=T), col='red')
        text(which.min(x), min(x, na.rm=T), paste(which.min(x), '-', format(min(x, na.rm=T), dig=3)), pos=4, col='red', cex=0.75)
    }
    ...
    AddLine(accuracy)
    if(!is.null(accuracy2)) AddLine(accuracy2, '#2BA2DE')
    abline(h=no.info, lty=3)
}